...
class NMT(nn.Module):
    ...
    def __init__(self, embed_size, hidden_size, vocab, dropout_rate=0.2, no_char_decoder=False):
        ...
        # Here attention we have a dropout_rate
    ...
    def forward(self, source: List[List[str]], target: List[List[str]]) -> torch.Tensor:
        ...
        target_padded_chars = self.vocab.tgt.to_input_tensor_char(target, device=self.device) # Tensor: (target, b, max_word_len)
        ...
        if self.charDecoder is not None:
            ...
            # Actually i dont know what's the usage of code below
            target_words = target_padded[1:].contiguous().view(-1)
            
            # Raw code, because of a RuntimeError we use reshape to instead reshape
#             target_chars = target_padded_chars[1:].view(-1, max_word_len)
    ...
    def step(self, Ybar_t: torch.Tensor,
        ...
        # compute the attention #dec_hidden has a shape of (b, h), enc_hiddens_proj has a shape of (b, src_len, h), return a e_t with shape (b, src_len)
        ...
        ### COPY OVER YOUR CODE FROM ASSIGNMENT 4
        # e_t has a shape of (b, src_len)
        alpha_t = F.softmax(e_t, dim=1)
        # bmm between alpha_t (b, src_len) and enc_hiddens (b, src_len, 2h), return a_t with shape (b, 2h)
