...
def pad_sents_char(sents, char_pad_token):
    ...
    word_pad_token = [char_pad_token for _ in range(max_word_length)]
    max_sentence_length = max(map(len, sents))
    sents_padded = [[w[:max_word_length]+[char_pad_token for _ in range(max_word_length-len(w))] for w in s]+[word_pad_token for _ in range(max_sentence_length-len(s))]for s in sents]
    ### END YOUR CODE
...
def pad_sents(sents, pad_token):
    ...
    for idx, sentence in enumerate(sents):
        sents_padded.append(sentence + [pad_token] * (max_len-lens[idx]))
