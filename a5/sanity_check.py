...
def question_1h_sanity_check():
    ...
    print("-"*80)
    print('Runing a random tensor for Question 1h')
    
    MAX_SENTENCE_LENGTH = 10
    random_X_conv_out = torch.randn(MAX_SENTENCE_LENGTH, BATCH_SIZE, EMBED_SIZE)
    
    highway = Highway(random_X_conv_out.size(-1))
    X_highway = highway(random_X_conv_out)
    
    output_size = tuple(X_highway.size())
    output_expected_size = (MAX_SENTENCE_LENGTH, BATCH_SIZE, EMBED_SIZE)
    
    assert output_size == output_expected_size, "output shape is incorrect: it should be:\n {} but is:\n{}".format(output_expected_size, output_size)
    print("Sanity Check Passed for Question 1g: Highway")
    print("-"*80)
...
def question_1i_sanity_check():
   ...
   """
    ...
    print("-"*80)
    print('Runing a random tensor for Question 1i')
    ...
    X_emb = torch.randn(MAX_SENTENCE_LENGTH, BATCH_SIZE, MAX_WORD_LENGTH, EMBED_CHAR_SIZE)
    X_reshaped = X_emb.permute(0,1,3,2)
    
    cnn = CNN(
        embed_char_size=EMBED_CHAR_SIZE, 
        filters_size=EMBED_WORD_SIZE,
    ...
    output_size = X_conv_out.size()
    output_expected_size = torch.Size((
        MAX_SENTENCE_LENGTH,
        BATCH_SIZE,
        EMBED_WORD_SIZE
    ))
    
    assert output_size == output_expected_size, "output size is incorrect: it should be:\n {} but is:\n{}".format(output_expected_size, output_size)
