...
class ParserModel(nn.Module):
    ...
    def embedding_lookup(self, t):
        ...
        x = self.pretrained_embeddings(t).view(t.size()[0], t.size()[1]*self.embed_size)
        ### END YOUR CODE
        return x
    ...
    def forward(self, t):
        ...
        # embeddings (batch_size, n_features*embeding_size)
        
        dropped = self.dropout(nn.functional.relu(self.embed_to_hidden(embeddings)))
        # dropped (batch_size, self.hidden_size)
        
        logits = self.hidden_to_logits(dropped)
        # logits (batch_size, self.n_classes)
        ### END YOUR CODE
        return logits
